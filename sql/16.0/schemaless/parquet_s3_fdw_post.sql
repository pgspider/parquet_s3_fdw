...
ALTER EXTENSION parquet_s3_fdw ADD TEXT SEARCH CONFIGURATION public.custom_search;
...
EXPLAIN (VERBOSE, COSTS OFF)
SELECT (t1.v->>'c1')::int8, (t2.v->>'c1')::int8 FROM ft1 t1 JOIN ft2 t2 ON ((t1.v->>'c1')::int8 = (t2.v->>'c1')::int8 AND CURRENT_USER = SESSION_USER) ORDER BY (t1.v->>'c3')::text, (t1.v->>'c1')::int8 OFFSET 100 LIMIT 10;
...
--Testcase 718:
EXPLAIN (VERBOSE, COSTS OFF)
SELECT * FROM local_tbl LEFT JOIN (SELECT ft1.* FROM ft1 INNER JOIN ft2 ON ((ft1.v->>'c1')::int8 = (ft2.v->>'c1')::int8 AND (ft1.v->>'c1')::int8 < 100 AND ((ft1.v->>'c1')::int8 - parquet_s3_fdw_abs((ft2.v->>'c2')::int)) = 0)) ss ON ((local_tbl.v->>'c3')::int8 = (ss.v->'c3')::int8) ORDER BY (local_tbl.v->>'c1')::int8 FOR UPDATE OF local_tbl;
--Testcase 719:
...
-- ====================================================================
-- parquet_s3_fdw not support use_remote_estimate and password_required options.
-- In local test file, the query will still success because it does not use a connection.
-- USER MAPPING is not necessary.
...
-- CREATE USER MAPPING FOR regress_view_owner_another SERVER parquet_s3_srv OPTIONS (password_required 'false');
...
-- \set var :PATH_FILENAME'/ported_postgres/child_local.parquet'
...
-- parquet_s3_fdw does not support truncate
-- truncate rem1;
