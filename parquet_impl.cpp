 ...
 *
 * IDENTIFICATION
 *		  contrib/parquet_s3_fdw/parquet_impl.cpp
 *
 *-------------------------------------------------------------------------
 */
...
public:
              ...
              bool use_threads)
    {
        ...
        elog(DEBUG1, "parquet_fdw: open Parquet file on S3. %s%s", dname, fname);
        pfree(dname);
        pfree(fname);

        openCore(std::move(this->reader_entry->file_reader->reader), cxt, tupleDesc, attrs_used, use_threads);
    }
              ...
              bool use_mmap)
    {
        ...
        openCore(std::move(reader), cxt, tupleDesc, attrs_used, use_threads);
    }

    void openCore(std::unique_ptr<parquet::arrow::FileReader> reader,
              MemoryContext cxt,
              TupleDesc tupleDesc,
              std::set<int> &attrs_used,
              bool use_threads)
    {
        ...
        this->allocator = new FastAllocatorS3(cxt);
    }
...
public:
    ...
    void add_file(const char *filename, List *rowgroups)
    {
        ...
        reader = new ParquetS3FdwReader(reader_id);
        if (s3_client)
            reader->open(filename, dirname, s3_client, cxt, tupleDesc, attrs_used, use_threads);
        else
...
private:
    ParquetS3FdwReader *get_next_reader()
    {
        ParquetS3FdwReader *r;
        ...
        r = new ParquetS3FdwReader(cur_reader);
        if (s3_client)
            r->open(files[cur_reader].filename.c_str(), dirname, s3_client, cxt, tupleDesc, attrs_used, use_threads);
        else
...
class MultifileMergeExecutionStateS3 : public ParquetS3FdwExecutionState
{
    struct FileSlot
    {
        int             reader_id;
...
private:
    std::vector<ParquetS3FdwReader *>   readers;
...
public:
    MultifileMergeExecutionStateS3(MemoryContext cxt,
        ...
        : cxt(cxt), tupleDesc(tupleDesc), attrs_used(attrs_used),
          ...
          slots_initialized(false), dirname(dirname), s3_client(s3_client)
    { }
    ...
    void add_file(const char *filename, List *rowgroups)
    {
        ...
        r = new ParquetS3FdwReader(0);
        if (s3_client)
            r->open(filename, dirname, s3_client, cxt, tupleDesc, attrs_used, use_threads);
        else
                         ...
                         Oid foreigntableid)
{
    ...
    {
        fdw_private->filenames = (List *) list_nth((List *) baserel->fdw_private, 0);
...
parquetBeginForeignScan(ForeignScanState *node, int eflags)
{
    ...
    try
    {
        ...
        {
            case RT_SINGLE:
                festate = new SingleFileExecutionStateS3(reader_cxt, dirname, s3client, tupleDesc,
                                                         attrs_used, use_threads,
                                                         use_mmap);
                break;
            case RT_MULTI:
                festate = new MultifileExecutionStateS3(reader_cxt, dirname, s3client, tupleDesc,
                                                        attrs_used, use_threads,
                                                        use_mmap);
                break;
            case RT_MULTI_MERGE:
                festate = new MultifileMergeExecutionStateS3(reader_cxt, dirname, s3client, tupleDesc,
                             ...
                             double *totaldeadrows)
{
    ...
    festate = new MultifileExecutionStateS3(reader_cxt,
                                          fdw_private.dirname,
...
parquetExplainForeignScan(ForeignScanState *node, ExplainState *es)
{
    ...
    rowgroups_list = (List *) list_nth(fdw_private, 6);
    ...
    foreach(lc, options_list)
    {
        ...
        if (strcmp(def->defname, "filename") == 0)
        {
            ...
            if (filename_provided)
                elog(ERROR, "parquet_fdw: either filename or dirname can be specified");
        ...
        else if (strcmp(def->defname, "dirname") == 0)
        {
            ...
            if (filename_provided)
                elog(ERROR, "parquet_fdw: ether filename or dirname can be specified");
            ...
            if (!IS_S3_PATH(dirname))
            {
                ...
                if (!S_ISDIR(stat_buf.st_mode))
                    ereport(ERROR,
                            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                             errmsg("parquet_fdw: %s is not a directory", dirname)));
...
get_filenames_in_dir(ParquetFdwPlanState *fdw_private)
{
    ...
    if (fdw_private->filenames == NIL)
        elog(ERROR, "parquet_fdw: object not found on %s", fdw_private->dirname);
}
