...
static void make_new_connection(ConnCacheEntry* entry, UserMapping* user, const char* use_minio);
static bool disconnect_cached_connections(Oid serverid);
static Aws::S3::S3Client* create_s3_connection(ForeignServer* server, UserMapping* user, const char* use_minio);
...
static Aws::S3::S3Client* s3_client_open(const char* user, const char* password, const char* use_minio);
 ...
 */
Aws::S3::S3Client*
parquetGetConnection(UserMapping* user, const char* use_minio)
	{
	bool		found;
 ...
 */
static void
make_new_connection(ConnCacheEntry* entry, UserMapping* user, const char* use_minio)
	{
...
create_s3_connection(ForeignServer* server, UserMapping* user, const char* use_minio)
	{
 ...
 */
static Aws::S3::S3Client*
s3_client_open(const char* user, const char* password, const char* use_minio)
	{
	...
	if ( strcmp(use_minio, "") != 0) {
		const Aws::String endpoint = use_minio;
